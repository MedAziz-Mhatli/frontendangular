...
this.editForm.patchValue({
  ...
  //edit
  onEditSaveCateg(): void {
    const formData = new FormData();
    formData.append('idCategorie', this.editForm.get('idCategorie').value);
    formData.append('nomCategorie', this.editForm.get('nomCategorie').value);
  
    // Vérifier si un nouveau fichier a été sélectionné
    const newImage = this.editForm.get('imageUrl').value;
    if (newImage instanceof File) {
      formData.append('imageUrl', newImage);
    } else {
      formData.append('imageUrl', this.editForm.get('imageUrl').value);
    }
    ...
    this.categorieService.updateCategorie(formData, httpOptions).subscribe(res => {
