...
this.editForm.patchValue({
  ...
  showDetails(categorie: Categorie): void {
    ...
    this.modalService.open('detailsModal', { size: 'lg' }).result.then(
      (result) => {
        // Modal fermée
      },
      (reason) => {
        // Modal fermée avec le bouton "Close" ou en cliquant en dehors de la modale
      }
    );
  }
  onSubmit() {
...
onEditSaveCateg() {
  ...
  const categorie = this.editForm.value as Categorie;
...
// }
  ...
  detailsRoww(row:Categorie, content: any) {
    this.selectedRowData = row; // set the selected row data
