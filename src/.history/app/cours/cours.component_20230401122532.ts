...
this.editForm.patchValue({
 ...
 // idVendeur: this.selectedRowDataa?.idVendeur,
  ...
  onEditSaveProduit(form: FormGroup): void {
    const editedProduit: Produit = {
      idProduit: form.value.idProduit,
      nomProduit: form.value.nomProduit,
      prixActuel: form.value.prixActuel,
      prixRreduction: form.value.prixRreduction,
      quantite: form.value.quantite,
      categorie: {idCategorie: form.value.idCategorie, nomCategorie: form.value.categorie},
    };
    this.produitsService.updateProduit(editedProduit).subscribe(() => {
      const index = this.produits.findIndex((p) => p.idProduit === editedProduit.idProduit);
      if (index > -1) {
        this.produits[index] = editedProduit;
        this.toastr.success('Le produit a été modifié avec succès.', 'Succès');
      }
      this.modal.dismiss('Cross click');
    }, (error) => {
      console.error(error);
      this.toastr.error('Une erreur est survenue lors de la modification du produit.', 'Erreur');
    });
  }
