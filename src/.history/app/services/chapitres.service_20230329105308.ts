...
export class ChapitresService {
  ...
  private Devoirs = 'http://localhost:8000/devParCh';
  private Exercices = 'http://localhost:8000/exParCh';
  private Qcm = 'http://localhost:8000/QcmParCh';
  ...
  public devoirs(chapitreId:string){
    return this.httpClient.get(`${this.Devoirs}/${chapitreId}`);
  }
  public cours(chapitreId:string){
    return this.httpClient.get(`${this.Cours}/${chapitreId}`);
  }
  public exercices(chapitreId:string){
    return this.httpClient.get(`${this.Exercices}/${chapitreId}`);
  }
  public qcm(chapitreId:string){
    return this.httpClient.get(`${this.Qcm}/${chapitreId}`);
  }
}
